...
async def get_disaster_event(event_id: str, _: Officer = Depends(get_current_officer)):
	return await get_document_or_404(DisasterEvent, event_id, "Disaster event not found")


@router.post("/", status_code=status.HTTP_201_CREATED)
async def create_disaster_event(payload: DisasterEventCreate, current_officer: Officer = Depends(require_admin)):
...
async def update_disaster_event(event_id: str, payload: DisasterEventUpdate, _: Officer = Depends(require_admin)):
	event = await get_document_or_404(DisasterEvent, event_id, "Disaster event not found")
