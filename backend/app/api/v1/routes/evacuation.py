...
async def get_evacuation_center(center_id: str, _: Officer = Depends(get_current_officer)):
	return await get_document_or_404(EvacuationCenter, center_id, "Evacuation center not found")


@router.post("/", status_code=status.HTTP_201_CREATED)
async def create_evacuation_center(payload: EvacuationCenterCreate, _: Officer = Depends(require_admin)):
...
async def update_evacuation_center(center_id: str, payload: EvacuationCenterUpdate, _: Officer = Depends(require_admin)):
	center = await get_document_or_404(EvacuationCenter, center_id, "Evacuation center not found")
